...
// List suppliers (admin/accountant/sales/warehouse_manager read)
export const getSuppliers = query({
    args: {},
    handler: async (ctx) => {
        const user = await getCurrentUser(ctx);
        if (
            !["admin", "accountant", "sales", "warehouse_manager"].includes(
                user.role
            )
        ) {
...
export const createSupplier = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .first();
        if (existing)
            throw new Error("Supplier with this tax code already exists");
...
export const updateSupplier = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const changes: any = {};
        if (args.companyName !== existing.companyName)
            changes.companyName = {
        ...
        if (args.taxCode !== existing.taxCode)
            changes.taxCode = { from: existing.taxCode, to: args.taxCode };
...
export const deleteSupplier = mutation({
    ...
    handler: async (ctx, args) => {
        const user = await getCurrentUser(ctx);
        if (user.role !== "admin")
            throw new Error("Only admin can delete suppliers");
    ...
    handler: async (ctx, args) => {
        ...
        for (const acc of accountants) {
            await notify(
                ...
                acc._id,
                "supplier_request",
                "New Supplier Creation Request",
                `${user.name} requested to create supplier: ${args.companyName}`
            );
        }
...
export const approveSupplierRequest = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.db.insert("supplier_history", {
            ...
            performedByRole: user.role,
            changes: {
                companyName: req.supplierData.companyName,
                taxCode: req.supplierData.taxCode,
