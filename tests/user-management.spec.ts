...
test.describe("User Management", () => {
    test.beforeEach(async ({ page }) => {
        ...
        const avatarButton = page.getByRole("button").filter({
            hasText:
                /[A-Z]{1,2}\s+.*(sales|admin|accountant|shipper|warehouse)/i,
        });
        if (await avatarButton.count()) {
            ...
            if (
                await profileTab.isVisible({ timeout: 1000 }).catch(() => false)
            ) {
                await profileTab.click();
            }
            const signOutBtn = page.getByRole("button", { name: "Sign out" });
            if (
                await signOutBtn.isVisible({ timeout: 1000 }).catch(() => false)
            ) {
        ...
        await page
            ...
            .fill("ngocanhnguyen.tayduong@gmail.com");
    ...
    test("admin can create a new user and the current admin profile stays unchanged", async ({
        page,
    }) => {
        ...
        const userMgmtTab = page.getByRole("button", {
        ...
        // Verify that a user row with the new email appears in the table
        const rowWithNewEmail = page.getByText(uniqueEmail);
        await expect(rowWithNewEmail).toBeVisible();

        // Verify welcome header text is unchanged (admin not mutated)
